// Bible Journaling Together V6 candidate: bounded spread queries and cache-aware Bible loading.
(()=>{
  'use strict';
  ...
  const DATA_URL = 'data/bible-kor.json?v=20260809';
  const VERSES_PER_SPREAD = 12;
  const COMMENTS_PER_SPREAD = 240;
  const REACTIONS_PER_SPREAD = 600;
  ...
  const MVP_DEBUG = false;
  let serverLoadVersion = 0;
  ...
  function rebuildVerseIndex(){
    verseById = new Map(state.bible.flatMap(book => book.chapters.flatMap(chapter => chapter.verses)).map(verse => [verse.id, verse]));
  }
  function allVerses(){ return [...verseById.values()]; }
  function findVerse(id){ return verseById.get(id); }
  ...
  async function loadServerData(){
    ...
    const loadVersion = ++serverLoadVersion;
    state.dbOnline = true;
    const [comments, verseReactions] = await Promise.all([fetchComments(), fetchVerseReactions()]);
    const commentReactions = await fetchCommentReactions(comments.map(comment => comment.id));
    if(loadVersion !== serverLoadVersion) return;
  ...
  async function fetchComments(){
    ...
    const verseIds = currentSpreadVerseIds();
    if(!verseIds.length) return [];
    let result = await db.from('comments').select(extended).in('verse_id', verseIds).is('deleted_at', null).order('created_at', {ascending:false}).limit(COMMENTS_PER_SPREAD);
    if(result.error) result = await db.from('comments').select(base).in('verse_id', verseIds).order('created_at', {ascending:false}).limit(COMMENTS_PER_SPREAD);
  ...
  async function fetchVerseReactions(){
    const verseIds = currentSpreadVerseIds();
    if(!verseIds.length) return [];
    const result = await db.from('verse_reactions').select('id, verse_id, anonymous_id, reaction_type, created_at').in('verse_id', verseIds).order('created_at', {ascending:false}).limit(REACTIONS_PER_SPREAD);
  ...
  async function fetchCommentReactions(commentIds){
    ...
    const result = await db.from('comment_reactions').select('id, comment_id, anonymous_id, reaction_type, created_at').in('comment_id', commentIds).order('created_at', {ascending:false}).limit(REACTIONS_PER_SPREAD);
  ...
  function currentSpreadVerseIds(){
    const offset = pageOffsetForSelected();
    return currentChapter().verses.slice(offset, offset + VERSES_PER_SPREAD).map(verse => verse.id);
  }
  ...
  function bindEvents(){
    ...
    capture(el.bookSelect, 'change', event => { state.bookIndex = Number(event.target.value); state.chapterIndex = 0; state.selected = currentChapter().verses[0]; renderAndRefresh(); });
    capture(el.chapterSelect, 'change', event => { state.chapterIndex = Number(event.target.value); state.selected = currentChapter().verses[0]; renderAndRefresh(); });
    ...
    document.addEventListener('click', event=>{
      ...
      if(event.target.closest('[data-reader-first]')){ stop(event); state.bookIndex=0; state.chapterIndex=0; state.selected=currentChapter().verses[0]; renderAndRefresh(); return; }
      if(event.target.closest('[data-reader-last]')){ stop(event); state.bookIndex=state.bible.length-1; state.chapterIndex=currentBook().chapters.length-1; state.selected=currentChapter().verses.at(-1); renderAndRefresh(); return; }
  ...
  function openVerse(id){
    ...
    const previousSpread = currentSpreadVerseIds().join(',');
    ...
    const nextSpread = currentSpreadVerseIds().join(',');
    if(previousSpread === nextSpread) render();
    else renderAndRefresh();
  }
  function moveChapter(step){
    ...
    state.bookIndex = bi; state.chapterIndex = ci; state.selected = currentChapter().verses[0]; renderAndRefresh();
  }
  ...
  async function submitComment(){
    ...
    state.comments = [result.data, ...state.comments.filter(comment => comment.id !== result.data.id)].slice(0, COMMENTS_PER_SPREAD);
    state.commentCounts.set(result.data.verse_id, (state.commentCounts.get(result.data.verse_id) || 0) + 1);
    openVerse(result.data.verse_id);
    render();
    setMessage('묵상을 저장했습니다.');
  }
