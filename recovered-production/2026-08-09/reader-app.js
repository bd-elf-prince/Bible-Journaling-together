(()=>{
  ...
  async function loadComments(verseIds){
    ...
    if(!unifiedResult.error){
      ...
      state.comments = (unifiedResult.data || [])
    ...
    console.info('[BJT] unified comments unavailable; using secured legacy comments');
    const securedLegacyResult = await withTimeout(
      db.rpc('get_public_comments', {
        p_verse_ids: ids
      }),
      9000,
      'secured legacy comments select'
    );
    if(requestId !== state.commentRequestId) return;
    if(!securedLegacyResult.error){
      state.commentBackend = 'legacy';
      state.comments = (securedLegacyResult.data || [])
        .map(comment => ({...comment, backend: 'legacy'}))
        .filter(comment => ids.includes(comment.verse_id));
      state.counts = countByVerse(state.comments);
      return;
    }

    console.info('[BJT] secured legacy comments unavailable; using transitional direct read');
    const legacyResult = await withTimeout(
      db
        .from('comments')
        .select('id, verse_id, user_name, content, created_at, updated_at, is_official')
        .in('verse_id', ids)
        .is('deleted_at', null)
        .order('created_at', {ascending:false}),
      9000,
      'legacy comments select'
    );
    if(requestId !== state.commentRequestId) return;
    if(legacyResult.error){
      console.warn('[BJT] comments select failed', legacyResult.error);
      if(!els.statusText?.classList.contains('is-visible')){
        setStatus('댓글 조회 권한을 확인해주세요.');
      }
      return;
    }
    state.commentBackend = 'legacy';
    state.comments = (legacyResult.data || [])
      .map(comment => ({...comment, has_edit_password:false, is_official:Boolean(comment?.is_official), backend: 'legacy'}))
      .filter(comment => ids.includes(comment.verse_id));
    state.counts = countByVerse(state.comments);
  }
