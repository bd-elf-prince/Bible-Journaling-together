...
(function(){
  ...
  const FALLBACK_BIBLE = [{
    ...
    chapters:[{number:1, subtitle:"태초의 빛과 창조의 질서", pageLeft:"창세기 · 첫째 날", pageRight:"창세기 · 좋았더라", verses:[
  ...
  const el = {};
  const state = {
    bible: normalizeBible(FALLBACK_BIBLE), bookIndex:0, chapterIndex:0,
    selected:null, comments:[], commentCounts:new Map(), commentReactions:[],
    liked:new Set(loadJson(STORAGE_LIKED, [])), reported:new Set(loadJson(STORAGE_REPORTED, [])),
    dbOnline:!!db, adminMode:new URLSearchParams(location.search).get("admin") === "1" || localStorage.getItem("bjt-admin-mode") === "true"
  };
  ...
  async function loadServerData(){
    ...
    saveJson(STORAGE_LIKED, [...state.liked]);
    render();
  }
  async function fetchComments(){
    ...
    if(result.error){ console.warn("comments select failed", result.error); setMessage("서버 댓글 테이블 확인 필요"); return []; }
  ...
  function keepValidSelection(){
    ...
    state.bookIndex = 0; state.chapterIndex = 0;
  ...
  function init(){
    ...
    state.selected = currentChapter().verses.find(verse => verse.number === 6) || currentChapter().verses[0];
    ensureChrome(); bindEvents(); exposeReader(); render();
    loadBibleData().then(loadServerData);
  }
  function exposeReader(){ window.V4Reader = {state, render, openVerse, loadBibleData, reloadComments:loadServerData, reportComment, adminDeleteComment}; }
  function ensureChrome(){ ensureTabs(); ensurePageDecor(); ensureFootbar(); ensureScopeLabel(); }
  function ensureTabs(){
    const panel = $("commentPanel"); if(!panel || panel.querySelector(".v4-panel-tabs")) return;
    const tabs = document.createElement("div"); tabs.className = "v4-panel-tabs";
  ...
  function ensureFootbar(){
    ...
    const bar = document.createElement("div"); bar.className = "reader-footbar";
  ...
  function ensureScopeLabel(){
    const title = document.querySelector(".comments-card .section-title"); if(!title || $("commentScopeLabel")) return;
  ...
  function bindEvents(){
    ...
    document.addEventListener("click", event => {
      ...
      const report = event.target.closest("[data-report-comment]"); if(report){ stop(event); reportComment(report.dataset.reportComment); }
      const del = event.target.closest("[data-delete-comment]"); if(del){ stop(event); adminDeleteComment(del.dataset.deleteComment); }
      const like = event.target.closest("[data-heart]"); if(like){ stop(event); reactToComment(like.dataset.heart); }
    }, true);
  }
  ...
  function renderPanel(){
    ...
    if(title) title.textContent = "이 절의 코멘트"; if(scope) scope.textContent = `${selected.number}절`;
    ...
    if(el.commentList) el.commentList.innerHTML = rows.length ? rows.map(renderCommentCard).join("") : `<p class="comment-empty">아직 이 절에는 서버 코멘트가 없습니다.</p>`;
  }
  function renderCommentCard(comment){
    ...
    const adminButton = state.adminMode ? `<button class="comment-admin-delete" type="button" data-delete-comment="${comment.id}">삭제</button>` : "";
  ...
  function renderForm(){
    const canUse = !!db;
    if(el.commentInput){ el.commentInput.disabled = !canUse; el.commentInput.placeholder = canUse ? "이 절에 대한 코멘트를 남겨보세요." : "Supabase 연결 후 코멘트를 남길 수 있어요."; }
    const submit = document.querySelector("#commentForm button[type='submit']"); if(submit) submit.disabled = !canUse;
  ...
  function recordSubmit(){ const now = Date.now(); const log = loadJson(STORAGE_SUBMIT_LOG, []).filter(time => now - time < 10 * 60 * 1000); log.unshift(now); saveJson(STORAGE_SUBMIT_LOG, log); }
  function reportAllowed(){
    const now = Date.now(); const log = loadJson(STORAGE_REPORT_LOG, []).filter(time => now - time < 60 * 60 * 1000);
    if(log.length >= 10) return false;
    log.unshift(now); saveJson(STORAGE_REPORT_LOG, log); return true;
  }
  async function submitComment(){
    if(!db){ setMessage("Supabase 연결을 확인해 주세요."); return; }
    const content = el.commentInput.value.trim().replace(/\s+$/g, "");
    if(!content) return;
    if(content.length > 1000){ setMessage("코멘트는 1000자 이하로 남겨주세요."); return; }
    if(!canSubmitNow()){ setMessage("잠시 후 다시 남겨주세요."); return; }
    ...
    if(result.error){ console.warn("comment insert failed", result.error); setMessage(`서버 저장 실패: ${result.error.message || "comments 권한/스키마 확인"}`); return; }
    recordSubmit(); el.commentInput.value = ""; await loadServerData(); openVerse(result.data.verse_id); setMessage("서버에 코멘트를 저장했습니다.");
  }
  async function reactToComment(commentId){
    ...
    state.commentReactions.unshift(result.data); state.liked.add(commentId); saveJson(STORAGE_LIKED, [...state.liked]); renderPanel();
  }
  async function reportComment(commentId){
    ...
    state.reported.add(commentId); saveJson(STORAGE_REPORTED, [...state.reported]); renderPanel(); setMessage("신고가 접수되었습니다.");
  }
  async function adminDeleteComment(commentId){
    ...
    await loadServerData(); setMessage("코멘트를 삭제했습니다.");
  }
